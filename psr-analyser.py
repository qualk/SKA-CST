   ...
   if per_packet:
      for packet in packets:
         ...
         with plt.style.context("ggplot"):
               ...
               ax.plot(channels, min_values, label='Min Value', marker='o')
               ax.plot(channels, max_values, label='Max Value', marker='o')
               ax.plot(channels, std_devs, label='Standard Deviation', marker='o')

               ax.set_xlabel('Channel')
               ax.set_ylabel('Value')
               ax.set_title(f'Statistics for Packet {packet.n_sequence + 1}')
               ax.legend()
               ax.grid(True)
               plt.show()
   else:
      ...
      with plt.style.context("ggplot"):
         ...
         ax.set_xlabel('Channel')
         ax.set_ylabel('Value')
         ax.set_title('Statistics Across All Packets')
         ax.legend()
         ax.grid(True)
         plt.show()
...
def plot_scale1_across_packets(packets: List[PulsarPacket]) -> None:
   ...
   with plt.style.context("ggplot"):
      ...
      plt.plot(packet_indices, scale1_values, marker='o', linestyle='-', color='b')
      
      plt.xlabel('Packet Index')
      plt.ylabel('Scale Value')
      plt.title('Scale Values Across Packets')
      plt.grid(True)
      plt.show()
...
if check_magic_words(udp_payloads):
   ...
   # plot_statistics(pulsar_packets, per_packet=False)
   plot_scale1_across_packets(pulsar_packets)
